...
export const resolveDid = async ({
  ...
  try {
    ...
    const didDocument = await didIo.get({did, options: didOptions});
    // FIXME services are not handled here yet
    return {
      statusCode: 200,
      resolutionResult: new DidResolutionResult({did, didDocument})
    };
  } catch(e) {
  ...
  if(error) {
    return {
      ...
      resolutionResult: new DidResolutionResult({did, error})
    };
  }
};
